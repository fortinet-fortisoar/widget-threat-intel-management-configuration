/* Copyright start
  ...
  (function () {
    angular
      .module('cybersponse')
      .directive('schedulePlaybookTim', schedulePlaybookTim);
  
    schedulePlaybookTim.$inject = ['$http', 'API', '$filter', '$window', 'CRUD_HUB', 'TimeZoneServices', 'SchedulesService',
      'localStorageService', 'currentPermissionsService'];
  
    function schedulePlaybookTim ($http, API, $filter, $window, CRUD_HUB, TimeZoneServices, SchedulesService,
      localStorageService, currentPermissionsService) {
      var directive = {
        restrict: 'A',
        scope: {
          viewWidgetVars: '<',
          connectorsToSchedule: '<'
        },
        controller: 'BaseCtrl',
        templateUrl: 'widgets/installed/threatIntelManagementConfiguration-1.0.0/widget/assets/services/schedulePlaybookTim.html',
        link: link
      };
      function link(scope) {
        scope.save = save;
